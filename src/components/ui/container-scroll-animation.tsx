...
export const ContainerScroll = ({
  ...
  const rotateTarget = useTransform(scrollYProgress, [0, 0.48, 1], [18, 0, -18]);
  const scaleTarget = useTransform(
    scrollYProgress,
    [0, 0.48, 1],
    isMobile ? [0.78, 0.92, 0.78] : [1.03, 1, 1.03],
  );
  const translateTarget = useTransform(
    scrollYProgress,
    [0, 0.48, 1],
    [0, -48, -96],
  );
  const springConfig = { stiffness: 80, damping: 24, mass: 0.7 };
  const rotate = useSpring(rotateTarget, springConfig);
  const scale = useSpring(scaleTarget, springConfig);
  const translate = useSpring(translateTarget, springConfig);

  return (
    <div
      className="relative flex h-[30rem] items-center justify-center p-0 md:h-[40rem]"
      ref={containerRef}
    >
      <div
